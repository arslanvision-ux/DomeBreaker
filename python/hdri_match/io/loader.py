...
def load_exr_to_numpy(file_path: str) -> np.ndarray:
    ...
    Attempts multiple backends in order: rawpy → OpenImageIO → OpenCV → ImageIO → PIL → OpenEXR → exr_pure.
    """
    errors = []
    ...
    # --- OpenImageIO (industry standard, native in Houdini/Nuke/VFX DCCs) ---
    try:
        ...
        if inp is not None:
            ...
            if data is not None:
                ...
                return arr
            errors.append("OpenImageIO: read_image returned None")
        else:
            errors.append(f"OpenImageIO: open failed ({oiio.geterror()})")
    except Exception as e:
    ...
    # --- PIL / Pillow (for LDR or common formats) ---
    try:
        from PIL import Image
        with Image.open(file_path) as p_img:
            img = np.array(p_img.convert("RGB"), dtype=np.float32) / 255.0
            return img
    except Exception as e:
    ...
    # --- Pure-Python EXR reader fallback ---
    if ext == '.exr':
        try:
