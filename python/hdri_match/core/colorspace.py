...
class ColorSpaceManager:
    ...
    def transform_image(self, image: np.ndarray, src_space: str, dst_space: str) -> np.ndarray:
        ...
        try:
            ...
            def resolve_space(space_name):
                ...
                aliases = {
                    "Linear": ["scene_linear", "linear", "Linear Rec.709 (sRGB)", "Utility - Linear - sRGB", "Utility - Linear - Rec.709", "lin_srgb", "ACES - ACEScg"],
                    "sRGB": ["srgb", "sRGB - Texture", "Utility - sRGB - Texture", "Output - sRGB", "sRGB - Texture"],
                    "Rec709": ["rec709", "Gamma 2.4 Rec.709 - Texture", "Utility - Rec.709 - Texture", "Output - Rec.709", "Rec.709 - Texture"],
                    "Rec.709": ["rec709", "Gamma 2.4 Rec.709 - Texture", "Utility - Rec.709 - Texture", "Output - Rec.709", "Rec.709 - Texture"],
                    "ACEScg": ["ACES - ACEScg", "ACEScg", "acescg", "cg"],
                    "ACES2065-1": ["ACES - ACES2065-1", "ACES2065-1", "aces2065-1"]
                }
    ...
    def _fallback_transform(self, image: np.ndarray, src: str, dst: str) -> np.ndarray:
        """Built-in mathematical fallback for ACEScg, ACES2065-1, sRGB, and Rec709."""
        ...
        def norm(s):
            ...
            if "2065" in s or ("aces" in s and "cg" not in s and "cc" not in s): return "aces2065-1"
        ...
        if src_n == "acescg":
            ...
            rgb = np.dot(rgb, M.T)
        elif src_n == "aces2065-1":
            # ACES2065-1 (AP0) to Linear sRGB Matrix
            M = np.array([
                [ 2.521686, -1.134131, -0.387555],
        ...
        if dst_n == "acescg":
            ...
            rgb = np.dot(rgb, M.T)
        elif dst_n == "aces2065-1":
            # Linear sRGB to ACES2065-1 (AP0) Matrix
            M = np.array([
                [0.439633, 0.382989, 0.177378],
                [0.089776, 0.813441, 0.096783],
                [0.017541, 0.111546, 0.870913]
            ], dtype=np.float32)
            rgb = np.dot(rgb, M.T)
