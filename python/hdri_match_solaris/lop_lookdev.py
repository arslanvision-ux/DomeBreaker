...
def create_lookdev_rig_node(
    ...
    and optional Macbeth ColorChecker chart mounted on a physically proportioned tripod stand.
    ...
    parms_to_add = [
        hou.ToggleParmTemplate("lookdev_en", "Enable Lookdev Rig", default_value=bool(lookdev_en)),
        hou.FloatParmTemplate("pos_x", "Position X", 1, default_value=[float(pos[0])], min=-50.0, max=50.0),
        hou.FloatParmTemplate("floor_y", "Floor Y (m)", 1, default_value=[float(pos[1])], min=-50.0, max=50.0),
        hou.FloatParmTemplate("pos_z", "Position Z", 1, default_value=[float(pos[2])], min=-50.0, max=50.0),
        hou.FloatParmTemplate("stand_height", "Stand Height (m)", 1, default_value=[float(stand_height)], min=0.0, max=10.0),
        hou.FloatParmTemplate("sphere_radius", "Sphere Radius (m)", 1, default_value=[float(radius)], min=0.01, max=5.0),
        hou.FloatParmTemplate("spacing", "Sphere Spacing (m)", 1, default_value=[float(spacing)], min=0.05, max=10.0),
        hou.FloatParmTemplate("rig_scale", "Rig Scale", 1, default_value=[float(rig_scale)], min=0.1, max=10.0),
        hou.ToggleParmTemplate("include_stand", "Include Stand Post", default_value=bool(include_stand)),
        hou.ToggleParmTemplate("include_white", "Include Matte White Sphere", default_value=bool(include_white)),
        hou.ToggleParmTemplate("include_macbeth", "Include Macbeth Chart", default_value=bool(include_macbeth)),
    ]

    modified_ptg = False
    for pt in parms_to_add:
        existing = ptg.find(pt.name())
        if not existing:
            ptg.append(pt)
            modified_ptg = True
        else:
            # Upgrade existing template with wider min/max limits
    ...
    if node.parm("lookdev_en"): node.parm("lookdev_en").set(bool(lookdev_en))
...
if stage:
    root = "{root_path}"
    lookdev_en = bool(node.parm("lookdev_en").eval() if node.parm("lookdev_en") else {lookdev_en})
    if not lookdev_en:
        ...
        eff_radius = max(0.005, base_r * rig_scale)
        eff_spacing = max(0.01, base_sp * rig_scale)
        eff_ball_y = floor_y + (base_h * rig_scale)
        crossbar_y = max(floor_y, eff_ball_y - eff_radius)
        mast_h = max(0.0, crossbar_y - floor_y)

        for path in [root, f"{{root}}/materials", f"{{root}}/spheres", f"{{root}}/stand", f"{{root}}/macbeth"]:
            if not stage.GetPrimAtPath(path).IsValid():
                UsdGeom.Scope.Define(stage, path)

        # Remove any legacy or obsolete prim paths from earlier revisions
        legacy_paths = [
            f"{{root}}/spheres/chrome", f"{{root}}/spheres/grey", f"{{root}}/spheres/white",
            f"{{root}}/stand/post", f"{{root}}/stand/base", f"{{root}}/ground_plane",
            f"{{root}}/materials/ground_mtl"
        ]
        ...
        # 3. Physically Proportioned Calibration Stand
        if include_stand and mast_h > 0.05:
            # Telescopic Central Mast
            mast = UsdGeom.Cylinder.Define(stage, f"{{root}}/stand/mast")
            mast.GetRadiusAttr().Set(max(0.008, 0.018 * rig_scale))
            mast.GetHeightAttr().Set(mast_h)
            mast.GetAxisAttr().Set("Y")
            xf_m = UsdGeom.Xformable(mast.GetPrim())
            xf_m.ClearXformOpOrder()
            xf_m.AddTranslateOp().Set(Gf.Vec3d(pos_x, floor_y + mast_h * 0.5, pos_z))
            UsdShade.MaterialBindingAPI.Apply(mast.GetPrim()).Bind(s_mtl)

            # Tripod Base Legs (touching the floor at floor_y)
            if mast_h > 0.15:
                leg_radius = max(0.006, 0.012 * rig_scale)
                foot_spread = max(0.12, min(0.45 * rig_scale, mast_h * 0.5))
                collar_h = min(0.30 * rig_scale, mast_h * 0.4)
                for i in range(3):
            ...
            # Clean up all stand prims if disabled or sitting on floor
            if stage.GetPrimAtPath(f"{{root}}/stand").IsValid():
                stage.RemovePrim(f"{{root}}/stand")

        # 4. Optional Macbeth ColorChecker Chart (mounted below crossbar)
        if include_macbeth and mast_h > 0.2:
